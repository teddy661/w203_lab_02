...
car_sales$my_category <- as.factor(car_sales$my_category)
```
...
```
```{r}
car_base_cat_ed <- car_sales %>% lm(ln_price_000 ~ ln_power_perf + my_category, data = .)
...
car_sales[,"my_category"] <- relevel(factor(car_sales$my_category), ref = "L")
...
power_price_catogory <- ggplot(car_sales_final, aes(x=ln_power_perf, y=ln_price_000, color = my_category)) +  geom_point()+ geom_smooth(method=lm)
power_price <- ggplot(car_sales_final, aes(x=ln_power_perf, y=ln_price_000)) +  geom_point()+ geom_smooth(method=lm)
power_sqrt_price <- ggplot(car_sales_final, aes(x=power_perf_sqrt, y=ln_price_000)) +  geom_point()+ geom_smooth(method=lm)
power_sqrt_price_catogory <- ggplot(car_sales_final, aes(x=power_perf_sqrt, y=ln_price_000, color = my_category)) +  geom_point()+ geom_smooth(method=lm)
...
```
```{r}
car_sales_final <- car_sales[!is.na(car_sales$ln_power_perf),]
...
#Base model version is Luxury in the "my_category"
car_sales_final[,"my_category"] <- relevel(factor(car_sales_final$my_category), ref = "L")
