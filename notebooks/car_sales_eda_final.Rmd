---
title: "Car Sales EDA FInal Edition"
...
```{r convert columns to appropriate types}
# Convert to Factors
...
car_sales$latest_launch <- as.POSIXct(car_sales$latest_launch, 
                                      format = "%m/%d/%Y")
```
```{r mutate incoming data and add our required study parameters}
car_sales <- car_sales %>%
  mutate(
...
complete_xy_plots <- ggarrange(
  ln_price_vs_ln_power_perf, ln_price_vs_ln_vehicle_size, ln_price_vs_vehicle_size
)
complete_xy_plots
```

```{r proposed first model, include=FALSE}
...
```{r proposed second model, include=FALSE}
...
car_base_size_se <- car_base_size %>%
  ...
  diag() %>%
  sqrt()
stargazer(car_base_size, type = "text", se = list(car_base_size))
...
```{r final regression table, message=FALSE, echo=FALSE, results='asis'}
stargazer(car_price_base, car_base_size, car_size_tier,
  ...
  dep.var.caption = "Output Variable: natural log of price in thousands of dollars",
  dep.var.labels = "",
  star.cutoffs = c(0.05, 0.01, 0.001),
  covariate.labels = c("Natural Log Power Performance Ratio", "Economy Tier Vehicles", "Mid Tier Vehicles", "Vehicle Size", "constant"),
