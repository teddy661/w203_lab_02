...
hist_curb_weight <- ggplot(car_sales, aes(x = curb_weight)) +
  geom_histogram(bins = 60)
hist_ln_curb_weight <- ggplot(car_sales, aes(x = ln_curb_weight)) +
  geom_histogram(bins = 60)
```
\newpage
...
complete_histograms <- ggarrange(
  ...
  hist_curb_weight, hist_ln_curb_weight
)
complete_histograms
```

```{r box plots for vehicle region, echo=FALSE}
vehicle_tier_boxplot <- ggplot(car_sales, aes(x = region, y = price_000)) +
  labs(title = "Price by Region", x = "Region", y = "Price in Thousands") +
  geom_boxplot(
...
```
```{r box plots for vehicle region, echo=FALSE}
vehicle_tier_boxplot <- ggplot(car_sales, aes(x = region, y = curb_weight)) +
  labs(title = "Curb Weight by Region", x = "Region", y = "Thousands of Lbs") +
  geom_boxplot(
    outlier.colour = "red", outlier.shape = 8,
    outlier.size = 4,
    notch = FALSE,
...
ln_price_vs_hp_per_liter <- ggplot(car_sales, aes(x = ln_hp_per_liter, y = ln_price_000)) +
  geom_point() +
  geom_smooth(method = lm, formula = y ~ x)
ln_price_vs_region <- ggplot(car_sales, aes(x = region, y = ln_price_000)) +
  geom_point() +
  geom_smooth(method = lm, formula = y ~ x)
ln_price_vs_weight <- ggplot(car_sales, aes(x = curb_weight, y = ln_price_000)) +
  geom_point() +
  geom_smooth(method = lm, formula = y ~ x)
ln_price_vs_ln_weight <- ggplot(car_sales, aes(x = ln_curb_weight, y = ln_price_000)) +
  geom_point() +
  geom_smooth(method = lm, formula = y ~ x)
```
...
complete_xy_plots <- ggarrange(
  ln_price_vs_ln_power_perf, ln_price_vs_ln_vehicle_size, ln_price_vs_vehicle_size, ln_price_vs_sqr_vehicle_size, ln_price_vs_hp_per_liter, ln_price_vs_region, ln_price_vs_weight, ln_price_vs_ln_weight
)
complete_xy_plots
```
...
ggplot(car_sales, aes(x = ln_power_perf, y = ln_price_000, color = region)) +
  geom_point() +
  geom_smooth(method = lm, formula = y ~ x) +
  labs(title = "Figure 3: Car Price and Power Performance Plot by Manufacturer Region", x = "Power Performance (in natural log)", y = "Price in Thousands (in natural log)") + 
  scale_color_manual(
...
```
```{r proposed third model, include=TRUE}
...
plot(3, main = 3)
plot(4, main = 4)
car_size_tier <- car_sales %>% lm(ln_price_000 ~ ln_power_perf + region + curb_weight, data = .)
...
stargazer(car_price_base, car_base_size, car_size_tier,
  ...
  covariate.labels = c("Power Performance Ratio (in natural log)", "Asia", "Europe", "Weight", "Constant"),
  notes = "\\parbox[t]{.55\\textwidth}{$HC_1$ robust standard errors in parentheses. American Vehicles are the base Tier}", notes.align = "l"
)
```
