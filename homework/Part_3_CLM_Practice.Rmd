...
```{r}
video <- video %>% 
  mutate(ln_views = log(views)) %>% drop_na(rate) %>% drop_na(length)
```
...
```{r warning=FALSE}
p5 <- ggplot(data = video, aes(x = video$rate, y = video$ln_views)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  labs(title = "Association between log Views and Rate", x = "Rate", y = "Log Views")

p6 <- ggplot(data = video, aes(x = video$length, y = video$ln_views)) +
  geom_point() +
  geom_smooth(se = FALSE) +
...
VIF_video <- ols_vif_tol(lm_video)
VIF_video
```


1. Evaluate the **IID** assumption.
The list of video contains videos sampled from YouTube using a crawling algorithm in which it starts with a set of  videos from the list of "Recently Featured", "Most Viewed", "Top Rated" and "Most Discussed", for "Today", "This Week", "This Month" and "All Time" and then the algorithm uses this list to find more related videos. We can argue that the initial set of videos are not independently sampled. For example, if the sample time frame is around election time, we would expect that "Recently Featured" videos are more likely to related to the topic of election. In addition, the crawl algorithm is adding videos into the list by finding videos that are directly related the initial set of videos. Therefore, by the nature of the sampling process, the list of videos is not IID but are related to each other.  

2. Evaluate the **No perfect Colinearity** assumption.
We have 2 methods to assess the **No perfect colinearity** assumption. First we can look at the scatter plot of these two variable to see if there is any obvious linear relationship. Second, we can compute the correlation between these two variable using Pearson's correlation method: 
...
plot_rate_vs_residuals <- video %>% ggplot(aes(x = rate, y = lm_video_residuals))+
  geom_point() + stat_smooth() + labs( title = "residuals vs rate")

plot_length_vs_residuals <- video %>% ggplot(aes(x = length, y = lm_video_residuals))+
  geom_point() + stat_smooth() + labs( title = "residuals vs length")

plot_prediction_vs_residuals <- video %>% ggplot(aes(x = lm_video_predictions, y = lm_video_residuals))+
  geom_point() + stat_smooth() + labs( title = "residuals vs prediction")


plot_rate_vs_residuals|plot_length_vs_residuals|plot_prediction_vs_residuals
...
```
From the residuals versus fitted values plot above, we see that as we are going from prediction with low values of 5 to high values of 8, the residual errors increase significantly. Additionally, we can use the Breusch-Pagan test where the null hypothesis is that homoscedasticity is present to evaluate our assumption. The test results in BP of 128.39 with p values of < 2.2e-16 which means that we can reject the null hypothesis. Therefore, we can conclude that heteroscedascity is present int he regression model. 

5. Evaluate the **Normally Distributed Errors:** assumption.

```{r q-q plot of residuals}
qqnorm(resid(lm_video)); qqline(resid(lm_video))
```


From the Q-Q plot of residual above showed that standard errors are fairly normally distributed. Thus we can say that there is no problem with the **Normally Distributed Errors:** assumption. 
