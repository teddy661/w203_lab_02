...
- We performed extensive EDA work to understand the sample collection process and the dataset. During our data wrangling process, we took the following actions to improve the data quality and model interpretability.  The original dataset has 9618 observations, and our final dataset has 9558 observations after removing 60 observations. 
  ...
  - Removed 51 observations with video length more than 11 minutes (considered as outliers) to address the multimodal distribution issues (see more details in #1 below)
...
- We conducted assessment of the five assumptions for the OLS regression below:
...
- With our EDA data wrangling work, the Linear Conditional Expectation assumption is generally met in our final model. Note that this assumption would have been violated if we were to use the original 9618 observations in the OLS regression model. 
...
- To evaluate this assumption, we plotted the residuals against the predicted views and evaluated whether the conditional variance is constant. Our visual observation is that the variance is generally constant, no clear evidence of heteroskedasticity. If we were to use the original 9618 observations, there were strong evidences that the variance increased as fitted value increased, which suggested the problem of heteroskedasticity.  Our final dataset appears to fix the problem of heteroskedasticity in the original dataset.
...
- Additionally, we performed the Breusch-Pagan (BP) test where the null hypothesis is that homoscedasticity is present. with BP value of 3.16 and p-value of 0.20, we fail to reject the null hypothesis of Homoskedastic Errors.  Please note that with the original dataset, the model would have BP value of 128.39 and p values less than 2.2e-16, which shows strong evidence that heteroskedasticity exists in the regression model. So our final model does not violate the Homoskedastic Errors assumption. 
...
```
Finally, please see our final model regression table below:
