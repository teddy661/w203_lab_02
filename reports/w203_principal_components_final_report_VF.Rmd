...
#
# Read input file from .csv located in data. Relative path to project file
# which was not submitted with this .Rmd
#
...
#
# Relabel column headers to values to be used later
#
...
#
# Convert columns to Factors for use as categorical varaiables in 
# linear regression
#
...
# this value isn't used in the final model. 
car_sales[, "vehicle_tier"] <- relevel(as.factor(car_sales$vehicle_tier),
  ref = "L"
)
#
# Convert latest_launch to R date variables for examination
#
...
#
# Create multiple columns from base data for EDA analysis
#
car_sales <- car_sales %>%
  mutate(
...
#
# Normalize days since last refresh to highest value.
#
car_sales$refresh_normalized <-
  car_sales$days_since_refresh / max(car_sales$days_since_refresh)

#
# During the EDA work, added a dummy variable to categorize power performance into above and below the mean value.  
#
...
#
# histograms for key variables from EDA work, not displayed in final report shows
# right skewed data (price and power performance ratio) and log transform shifting to a normal distribution
#
...
#
# Display similar histograms as above (in different format). Not included in the final report.  Included in the presentation slides (appendix section)
#
...
We utilized a car sales data set sourced from Analytixlabs by Kaggle\footnote{https://www.kaggle.com/datasets/gagandeep16/carsales}. The data set was assembled in 2013 and contained the characteristics of 157 different vehicle models from thirty (30) automobile manufacturers. This continuous dataset includes the critical variables needed for our study. This dataset represents the major global vehicle manufacturers across the three continents (Europe, the US, and Asia). It does not include some regional manufacturers that may have a significant presence in their local markets. We conducted exploratory data analysis (EDA) and developed the regression models on the entire data set. An alternative approach was to use a 30% training and 70% testing data split. However, given our sample size, the alternative approach would result in a too-small training dataset, which may not result in consistent model estimates. Therefore, we chose the entire dataset for our EDA and modeling work and evaluated the critical assumptions in the limitations section.
...
#
# Output Variable vs Input Variable. Displayed in final report
#
...
```{r price and power performance ratio by two regimes (at/above and below mean), warning=FALSE, echo=FALSE, include=FALSE}
#
# Reviewed the correction of power and power performance ratio by the two regimes. Consistent patterns
#
ggplot(car_sales, aes(x = ln_power_perf, y = ln_price_000, color = car_sales$cat_power_perf)) +
  geom_point() +
  geom_smooth(method = lm, formula = y ~ x) +
  labs(x = "Power Performance (in natural log)", y = "Price in Thousands (in natural log)") +
  scale_color_manual(
    name = "Higher/Lower Power Performance Regimes",
    labels = c("Below Average", "At/Above Average"),
    values = c("red", "green")
  ) +
...
#
# Base Model using only the natural log of power performance ratio to predict MSRP
#
...
#
# Include Categorical Variable of Manufacturer Region e.g. USA, Asia, European
#
...
#
# Include vehicle curb weight in final model
#
...
#
# Homoskedastic test and conditional variance testing, not displayed in final report
#
